...
export const txExplainerUtils = {
    ...
    async decodeRevertReasonAsync(
        ...
        if (!_.isUndefined(result)) {
            const decodedRevertReason =
                abiDecoder.decodeCalldataOrThrow(result);
...
export class TxExplainer {
    ...
    constructor(provider: Provider, networkId: number) {
        ...
        abiDecoder.addABI(
            (AssetProxyOwner as any).compilerOutput.abi,
            "AssetProxyOwner"
        );
    ...
    public async explainTransactionAsync(
        ...
        const contract: OrderValidatorContract = await (
            this._contractWrappers.orderValidator as any
        )._getOrderValidatorContractAsync();
        const ordersAndTradersInfo =
            await contract.getOrdersAndTradersInfo.callAsync(
