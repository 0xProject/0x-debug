...
export class Enable extends Command {
    ...
    public async run(): Promise<void> {
        ...
        const tokenContract = new ERC20TokenContract(flags.token, provider, { from: selectedAddress });
        const result = await utils.awaitTransactionWithSpinnerAsync('Approve token for trading', () =>
            tokenContract.approve.awaitTransactionSuccessAsync(
